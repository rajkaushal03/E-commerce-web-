  ...
  return (
    ...
    >
      {newProduct.slice(0, 20).map((product) => {
        ...
        return (
          <div
            className="lg:h-64 h-52 2xl:h-96 md:h-64 border-2 border-black shadow-[0px_4px_16px_rgba(17,17,26,0.1),_0px_8px_24px_rgba(17,17,26,0.1),_0px_16px_56px_rgba(17,17,26,0.1)] cursor-pointer rounded-3xl flex flex-col 2xl:gap-6 md:gap-0 "
            key={product.id}
          >
            {/* Image */}
            <Link to="/detail" state={{ product }}>
              <div className="bg-white rounded-t-3xl px-2 h-full">
                <img
                  className="w-full 2xl:h-56 h-28 md:h-40 lg:h-40 p-3 relative hover:scale-110 transition-transform duration-1000 ease-in-out object-scale-down"
                  src={product.image}
                  alt={product.title
            ...
            <div className="flex flex-col justify-end h-full p-3 md:px-4  2xl:gap-3 md:gap-0">
              {/* Product Title */}
              <div className="2xl:text-md lg:text-md text-xs  tracking-tight h-1/2 text-wrap text-center">
                {product.title
                  .split(" ") // Split the description into an array of words
              ...
              >
                <span className="2xl:text-lg lg:text-sm text-xs font-bold">${product.price}</span>
                <button className="text-white btn md:btn-md  btn-sm bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg md:text-xs 2xl:text-sm  2xl:px-3 sm:px-2  py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
